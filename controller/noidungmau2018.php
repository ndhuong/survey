...
function danhSach(){
	...
	$danhsach_sql='SELECT * FROM noidungkhaosat2018 ORDER BY nd18_stt ASC';
	...
	if($danhsach_query){
		if(mysqli_num_rows($danhsach_query)<1){
			...
			while ($result = mysqli_fetch_array( $danhsach_query )){
				$stt=$result['nd_stt'];
				if($result['nd_loai']==1) $tmp= '  Câu trả lời Có / Không';
				elseif($result['nd_loai']==2) $tmp= ' Câu trả lời lựa chọn có chữ Khác';
				...
				$tmp5="onclick=\"return confirm('Bạn có chắc chắn xóa nội dung này không ?')\" ";
				$sua='<a href="./index.php?ylan=noidungmau2018&sua='.$id.'"> <span class="fa fa-pencil fa-1x"></span> </a>';
				$xoa='<a href="./index.php?ylan=noidungmau2018&xoa='.$id.'" '.$tmp5.' ><span class="fa fa-trash fa-1x"></span></a>';
				
				$danhsach=$danhsach.'<tr><td class="text-center">'.$stt.'</td><td>'.$result['nd_noidung'].'</td><td>'.$tmp.'</td><td>'.$result['nd_ten_bien'].'</td><td>'.$result['nd_ten_sql'].'</td><td class="text-center">'.$sua.' </td><td class="text-center">'.$xoa.'  </td></tr>';
			}
